...
int main(){
    ...
    for(int i=0; i<NUM_SAMPLES; i++){
        for(int j=0; j<16; j++){
            for(int k=0; k<16; k++){
                data.in[i][j][k] = train_inputs[i][j][k]; //train_inputs is a train data set. I made it array header file. So that I do not need I/O file system.
            }
        }
    }
    for(int i=0; i<NUM_SAMPLES; i++){
        for(int j=0; j<10; j++){
                data.tg[i][j] = train_outputs[i][j]; ////train_outputs is a train data set. I made it array header file. So that I do not need I/O file system.
        }
    }
    ...
    int model_size =dLayer_Save(&d_layer, model_data);  //store the trained weights and biases
    //I want to save it into array header file, so that I can use this array header file for inference and do not need I/O file system
    FILE *fw = fopen("trained_model.h", "w");
    if(!fw){
        printf("Error: cannot open trained_model.h for writing \n");
        return 1;
    }

    fprintf(fw, "#ifndef TRAINED_MODEL_H\n#define TRAINED_MODEL_H\n\n");

    fprintf(fw, "static const int CONV_FILTER_SIZE = %d;\n", conv_layer.filter_size);
    fprintf(fw, "static const float conv_filter[%d][%d] = {\n", conv_layer.filter_size, conv_layer.filter_size);
    for(int i=0; i<conv_layer.filter_size; i++){
        fprintf(fw, "    {");
        for(int j=0; j<conv_layer.filter_size; j++){
            fprintf(fw, "%ff", conv_layer.filter[i][j]);
            if(j<conv_layer.filter_size-1) fprintf(fw, ", ");
        }
        fprintf(fw, "}");
        if(i<conv_layer.filter_size-1) fprintf(fw, ",");
        fprintf(fw, "\n");
    }
    fprintf(fw, "};\n\n");


    fprintf(fw, "static const int MODEL_SIZE = %d;\n", model_size);
    fprintf(fw, "static const float trained_mode_data[%d] = {\n", model_size);

    for(int i=0; i<model_size; i++){
        fprintf(fw, "%ff", model_data[i]);
        if(i<model_size-1) fprintf(fw, ", ");
        if((i+1) % 8 == 0) fprintf(fw, "\n");
        
    }

    fprintf(fw, "\n};\n\n#endif // TRAINED_MODEL_H\n");
    fclose(fw);
    printf("Trained model saved to trained_model.h\n");
